...
def calculate_distance(known_width, pixel_width, focal_length):
    if pixel_width == 0:
	...
	for i in np.arange(0, predictions.shape[2]):
		...
		if confidence > args["confidence"]:
			...
			if CLASSES[idx]=="background":
				...
				known_width=5
			distance=calculate_distance(known_width,pixel_length,800)
			...
			cv2.putText(frame, str(distance), (startX, y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, COLORS[idx], 2)
